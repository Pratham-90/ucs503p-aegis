...
\begin{itemize}[leftmargin=0.7cm]
  ...
  \item creation of a single vault per Owner; the payload is encrypted \emph{in
    the Owner's browser} and uploaded as ciphertext only;
  \item configuration of a check-in interval and a grace period;
  \item designation of $N$ trustees (each enrolling a public key) and a
    threshold $K$ ($2 \le K \le N$);
  \item a scheduler that issues check-in prompts and, on missed check-in past
    the grace period, distributes each trustee's encrypted share blob;
  \item a one-action check-in confirmation for the Owner;
  \item client-side reconstruction: any $K$ trustees decrypt their blobs and
...
payload under the shared key --- all performed \textbf{client-side} (see the
Trust model).
...
\section{Trust model \BvrHint{\ldots who is trusted for what}}
Aegis adopts a \textbf{zero-knowledge server} design: the server is trusted for
\emph{scheduling and availability}, and \textbf{never for confidentiality}. All
payload cryptography --- symmetric-key generation, payload encryption, Shamir
splitting, and reconstruction --- is performed on clients (the Owner's and the
trustees' browsers). The server persists only the ciphertext payload, the $N$
trustee-encrypted share blobs, and non-secret metadata (\ReqID{FR-2},
\ReqID{FR-2a}, \ReqID{NFR-SEC-5}).

The boundary and its honest consequences:
\begin{itemize}[leftmargin=0.7cm]
  \item \textbf{The server alone can never read a payload.} A confidentiality
    breach requires the server \emph{and} at least $K$ trustees.
  \item \textbf{The server controls release timing.} It could hand over blobs
    early or refuse to release altogether. Early release nonetheless yields
    plaintext only if $\ge K$ trustees then collude; denial of release is an
    availability failure, not a confidentiality one (threat \texttt{T-7}).
  \item \textbf{Client code is server-delivered.} The zero-knowledge property
    holds against a passive or storage compromise of the server. It does
    \emph{not} hold against a malicious server that serves backdoored client
    code, which could exfiltrate the key at encryption time. This residual risk
    is stated plainly and is \textbf{not} mitigated in v1 (threat \texttt{T-4}).
\end{itemize}
The full per-threat analysis --- capabilities, impact, mitigation, and residual
risk --- is maintained in \texttt{docs/threat-model.md}.
...
\begin{description}[leftmargin=0cm,style=nextline]
  \item[\ReqID{FR-1} Registration \& authentication.] The system shall allow a
    ...
    \ReqID{NFR-SEC-3}).
  \item[\ReqID{FR-2} Vault creation \& client-side payload encryption.] An
    authenticated Owner shall be able to create their (single) vault. The
    payload (files and/or a text message) shall be encrypted \textbf{in the
    Owner's browser} under a locally generated symmetric key; only the resulting
    ciphertext and non-secret metadata are uploaded. The server shall never
    receive the plaintext payload or the symmetric key (see \ReqID{NFR-SEC-1},
    \ReqID{NFR-SEC-5}).
  \item[\ReqID{FR-2a} Client-side key splitting \& share encryption.] In the
    Owner's browser, the symmetric key shall be split into $N$ Shamir shares
    with threshold $K$ (\ReqID{FR-4}); each share shall then be encrypted under
    its designated trustee's enrolled public key before upload. The server shall
    receive only the $N$ trustee-encrypted share blobs --- never a plaintext
    share or the key (see \ReqID{NFR-SEC-5}).
  ...
  \item[\ReqID{FR-4} Trustee designation, enrolment \& threshold.] The Owner
    shall designate $N \ge 1$ trustees (by email) and set a threshold $K$ with
    $2 \le K \le N$. Each designated trustee shall enrol a \emph{public key}
    whose corresponding private key is generated on, and never leaves, the
    trustee's own device; these public keys are the inputs to the client-side
    split (\ReqID{FR-2a}), so the key is never split or held server-side. On the
    threshold the system shall \textbf{reject} any configuration with $K < 2$ (a
    single trustee must never suffice), shall \textbf{warn} the Owner when
    $K = N$ that the vault becomes unrecoverable if any one trustee is lost, and
    the configuration UI shall \textbf{display} the loss tolerance $N - K$ (the
    number of trustees that may be lost while the vault stays recoverable).
  ...
  \item[\ReqID{FR-6} One-action check-in confirmation.] The Owner shall be able
    ...
    \emph{Active}.
  \item[\ReqID{FR-7} Blob distribution on expiry.] If no valid check-in is
    ...
    and deliver to each trustee their own encrypted share blob (already
    encrypted to that trustee at upload, per \ReqID{FR-2a}) --- and shall do so
    \textbf{only then} (see \ReqID{NFR-REL-1}). The server distributes blobs; it
    never holds a plaintext share.
  \item[\ReqID{FR-8} Client-side $K$-of-$N$ reconstruction \& decryption.] After
    release, each trustee shall decrypt their blob locally with their private key
    to recover their plaintext share. Any $K$ trustees shall then combine shares
    \textbf{in the browser} to reconstruct the symmetric key and decrypt the
    payload; fewer than $K$ shall not permit decryption. The server performs no
    decryption and at no point sees a plaintext share, the key, or the payload
    (see \ReqID{NFR-SEC-5}).
\end{description}
...
\begin{description}[leftmargin=0cm,style=nextline]
  \item[\ReqID{NFR-SEC-1} Payload never in plaintext.] The payload shall never
    ...
    any vault; the plaintext key exists only transiently in \emph{client} memory
    (the Owner's browser at encryption, a trustee's browser at reconstruction)
    and never on the server (see \ReqID{NFR-SEC-5}).
  ...
  \item[\ReqID{NFR-SEC-5} Zero-knowledge server.] The server shall at no point
    possess sufficient material to decrypt a payload unaided. \emph{Measure:} an
    inspection of everything the server persists or receives (database rows,
    uploaded objects, request logs) finds, for every vault, only the ciphertext
    payload, the $N$ trustee-public-key-encrypted share blobs, and non-secret
    metadata --- no plaintext payload, no symmetric key, and no plaintext share.
    Decryption therefore requires material held only by clients and by $\ge K$
    trustees, never by the server alone (see the Trust model, and
...
diagram in \texttt{docs/diagrams/vault-lifecycle.md}. The trust boundary and
per-threat coverage are analysed in \texttt{docs/threat-model.md}, whose
mitigations cite these same identifiers.
